...
void vprint_message2(const char *const format, va_list args) {
  ...
  char *last = buffer + (len >= sizeof(buffer) - 3 ? sizeof(buffer) - 2 : len);
#ifdef _WIN32
  *(last - 1) = '\r';
  *last = '\n';
  *(last + 1) = '\0';
...
int _run_tests(const UnitTest *const tests, const size_t number_of_tests) {
  ...
  while (current_test < number_of_tests) {
    ...
    if (run_next_test) {
      int failed = _run_test(test->name, test->function, current_test, current_state,
